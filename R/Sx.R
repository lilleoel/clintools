...
Sx <- function(
   ...
   df.agg <- Z.aggregate(df,freq,blocksize,blockmin,by_type=c("max","mean"),n_vars=2)

   #CORRELATION ANALYSES
   df.agg <- Z.epochs(df.agg,epochsize,epochmin,overlapping)
   df.cor <- Z.cor(df.agg, cor_by = c("val1_mean","val2_max"),overlapping)
   results <- Z.cor_output(df, df.agg, df.cor, freq, output, cor_by = c("val1_mean","val2_max"), overlapping)
